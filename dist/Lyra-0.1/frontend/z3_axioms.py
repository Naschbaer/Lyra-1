...
def index(indexed, idx, result, types):
    """Constraints for index subscript: indexed[idx].

    Cases:
    - indexed: bytes                      => result: bytes
    - indexed: str                        => result: str
    - indexed: Tuple[T1, T2, T3, ..., Tn] => result: supertype(T1, T2, T3, ..., Tn)
    - indexed: List[T]                    => result: T
    - indexed: Dict[T1, T2]               => T2
    """
    tuples = list()
    for cur_len in range(1, len(types.tuples)):     # indexed can be a tuple of arbitrary length
        args = [getattr(types.type_sort, "tuple_{}_arg_{}".format(cur_len, i + 1))(indexed) for i in range(cur_len)]
        # indexed can be a tuple of arbitrary length, result is the super type of the elements of the tuple
        tuples.append(And(indexed == types.tuples[cur_len](*args), *[types.subtype(x, result) for x in args]))

    return [Or([
        And(indexed == types.string, types.subtype(idx, types.int), result == types.string),    # string
        And(indexed == types.bytes, types.subtype(idx, types.int), result == types.bytes),      # bytes
        And(indexed == types.list(result), types.subtype(idx, types.int)),                      # list
        indexed == types.dict(idx, result),                                                     # dictionary
    ] + tuples)]
